...
!!! info "Status: Live (reproduced 2026-05-23)"
    Verified against examon-core v0.5.0. Individual sub-sections carry their own status. Dashboards: Live for core Grafana; Beta for the 3D Digital Twin plugin (public release pending). Analyze: Beta; the Trino connector is shipped publicly but not bundled into the local chart. AI: Beta (public release pending).
