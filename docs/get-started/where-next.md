...
- **Analyze**: SQL through Trino from any BI tool, notebook, or scripting language, plus a legacy Python client for pre-Trino deployments. The [Analyze section](../users/analyze/index.md) covers the Trino schema, example queries, per-tool connection guides. Install Trino from [Add-ons → Trino](../administrators/add-ons/trino.md) (Docker Compose or Kubernetes overlay), then run the first query from [Query with Trino](../users/analyze/query-with-trino.md). The bundled [ExamonQL notebook](../users/analyze/Demo_ExamonQL.ipynb) uses the legacy `examon-client` interface against the ExaMon REST API and remains useful in pre-Trino deployments.
