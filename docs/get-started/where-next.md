...
- **Analyze**: SQL through Trino from any BI tool, notebook, or scripting language, plus a legacy Python client for pre-Trino deployments. The [Analyze section](../users/analyze/index.md) covers the Trino schema, example queries, per-tool connection guides. The [Trino Quickstart](../users/analyze/local-trino-quickstart.md) is the fastest way to a first SQL query, with paths for both a laptop-sized fresh install and adding the connector to an existing `trino/trino` release. The bundled [ExamonQL notebook](../users/analyze/Demo_ExamonQL.ipynb) uses the legacy `examon-client` interface against the ExaMon REST API and remains useful in pre-Trino deployments.
