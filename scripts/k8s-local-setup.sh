...
echo ""
echo "MQTT broker is exposed via K3d NodePort:"
echo "  mosquitto_sub -h localhost -p 1883 -t '#' -v"
echo ""
echo "Other services (use kubectl port-forward):"
echo "  Grafana:   kubectl port-forward svc/examon-grafana 3000:80 -n ${NAMESPACE}"
echo "  API:       kubectl port-forward svc/examon-examon-server 5000:5000 -n ${NAMESPACE}"
